...
func main() {
	...
	for r := 0; r < 100; r++ {
		go func() {
			total := 0
			for {
				// For each read we pick a key to access, Lock() the mutex to ensure exclusive
				// access to the state, read the value at the chosen key,
				...
				// This yielding is handled automatically with e.g. every channel operation
				// and for blocking calls like time.Sleep, but in this case we need to do it manually.
				runtime.Gosched()
			}
		}()
	}
